            ...
            for batch_ids, batch_attention_masks, batch_arg_indications, iter_batch_srl_labels, iter_target_s, iter_target_e, iter_target_srl, target_dicts in train_dataset:
                ...
                for i, (batch_srl_labels, batch_target_s, batch_target_e, batch_target_srl) in enumerate(
                    ...
                    if i != iter_size-1:
                        ...
                        if RL_FLAG:
                            # get next transition
                            probs_rl, next_trans = policy(batch_arg_indications, torch.concatenate([token_embs.detach(), srl_embs.detach()], dim=2))    # SRLモデルに影響を与えないようにdetach
                            
                            with torch.no_grad():   # 計算グラフを保持しつつ，以下の箇所では記録を行わない
                                # start
                                prob_s = start_decoder(torch.concatenate([token_embs, srl_embs], dim=2), batch_attention_masks[:,1:])
                                for j, srl_labels in enumerate(batch_srl_labels):
                                    filter_s = mk_filter_s(srl_labels[j], MAX_TOKEN, lab2id)
                                    prob_s[j] = torch.where(filter_s.unsqueeze(0), prob_s[j], torch.tensor(float('-inf'), device="cuda:0"))
                                starts = torch.argmax(prob_s, dim=1)[0]
                                start_tokens = token_embs[torch.arange(batch_size), starts].unsqueeze(1) # [batch, 1, hidden]
                                start_tokens_expanded = start_tokens.expand(-1, token_len, -1) # [:,target,:]と[[0,1,...],target]は，別物であることに注意
                                ...
                                for j, srl_labels in enumerate(batch_srl_labels):
                                    ...
                                    prob_e[j] = torch.where(filter_e.unsqueeze(0), prob_e[j], torch.tensor(float('-inf'), device="cuda:0"))
                                ends = torch.argmax(prob_e, dim=1)[0]
                                end_tokens = token_embs[torch.arange(batch_size), ends].unsqueeze(1) # [batch, 1, hidden]
                                
                                # srl
                                start_end_vecs = torch.concatenate([start_tokens, srl_embs[torch.arange(batch_size), starts].unsqueeze(1),
                    ...
                    # 反復試行の最終であれば
                        ...
                        if RL_FLAG:
                            # get next transition
                            probs_rl, next_trans = policy(batch_arg_indications, torch.concatenate([token_embs.detach(), srl_embs.detach()], dim=2))
