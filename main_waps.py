...
def test(sentence_encoder, srl_encoder, start_decoder, end_decoder, srl_decoder, dataset, lab2id, id2lab):
    ...
    with torch.no_grad():
        for batch_ids, batch_attention_masks, batch_arg_indications, partial_srl_prev_labels, iter_batch_srl_labels, iter_target_s, iter_target_e, iter_target_srl in dataset:
            batch_ids, batch_attention_masks = batch_ids.to(device), batch_attention_masks.to(device)
            partial_srl_prev_labels = [srl_prev_labels.to(device) for srl_prev_labels in partial_srl_prev_labels]
            ...
            for _ in range(MAX_ITER):
                srl_embs = srl_encoder(srl_labels, partial_srl_prev_labels, batch_attention_masks[:,1:])       # [batch, token, hidden]
...
if __name__ == '__main__':
    ...
    train_df = train_df.sample(frac=0.01, random_state=0).reset_index(drop=True)
    valid_df = valid_df.sample(frac=0.01, random_state=0).reset_index(drop=True)
    test_df = test_df.sample(frac=0.01, random_state=0).reset_index(drop=True)
    ...
    srl_config = BertConfig(
        ...
        hidden_size=256*2,       # 768 の 1/3. srl*2
        num_attention_heads=8    # 1つのヘッドが64になるように設定
    )
    decoder_config1 = BertConfig(
        num_hidden_layers=1,     # Transformerのレイヤー数
        hidden_size=768+256*2,   # token + srl*2
        num_attention_heads=20   # 1つのヘッドが64になるように設定
    )
    decoder_config2 = BertConfig(
        num_hidden_layers=1,       # Transformerのレイヤー数
        hidden_size=768+256*2+768, # token + srl*2 + token_s
    ...
    srl_encoder = SRLEncoder2(srl_config, len(lab2id)).to(device)
    ...
    if TRAIN_FLAG:
        ...
        print(f'Rate : 5e-5, 1e-4, 1e-4, 1e-4, 1e-4')
        ...
        for epoch in range(100):
            ...
            for batch_ids, batch_attention_masks, batch_arg_indications, partial_srl_prev_labels, iter_batch_srl_labels, iter_target_s, iter_target_e, iter_target_srl in train_dataset:
                #print('iter_batch_srl_labels = ', iter_batch_srl_labels.shape)
                # gpuへ転送
                batch_ids, batch_attention_masks, batch_arg_indications = batch_ids.to(device), batch_attention_masks.to(device), batch_arg_indications.to(device)
                partial_srl_prev_labels = [srl_prev_labels.to(device) for srl_prev_labels in partial_srl_prev_labels]
                ...
                for i, (batch_srl_labels, batch_target_s, batch_target_e, batch_target_srl) in enumerate(
                    ...
                    if i != iter_size-1:
                        srl_embs = srl_encoder(batch_srl_labels, partial_srl_prev_labels, batch_attention_masks[:,1:])       # [batch, token, hidden]
                        ...
                        # RL
                        if RL_FALG:
                            probs_rl, next_action = policy(batch_arg_indications, torch.concatenate([token_embs.detach(), srl_embs.detach()], dim=2))
                            preds = list(zip(torch.argmax(prob_s, dim=1).reshape(-1), torch.argmax(prob_e, dim=1).reshape(-1), torch.argmax(prob_srl, dim=1).reshape(-1)))
                            targets = list(zip(batch_target_s.reshape(-1), batch_target_e.reshape(-1), batch_target_srl.reshape(-1)))
                            rewards = policy.cal_rewards(preds, targets, lab2id)
                        ...
                        batch_loss_multi = (batch_loss_s + batch_loss_e + batch_loss_srl)
                        ...
                        if RL_FALG:
                            probs_rl, next_action = policy(batch_arg_indications, torch.concatenate([token_embs.detach(), srl_embs.detach()], dim=2))
                            preds = list(zip(torch.argmax(prob_s, dim=1).reshape(-1), [-1]*batch_size, [-1]*batch_size))
                            targets = list(zip(batch_target_s.reshape(-1), batch_target_e.reshape(-1), batch_target_srl.reshape(-1)))
                            rewards = policy.cal_rewards(preds, targets, lab2id)
                ...
                if (i + 1) % ITERS_TO_ACCUMULATE == 0:
                    ...
                    if RL_FALG:
                        optimizer2.step()
                        policy.zero_grad()
            print(f"############## epoch {epoch} \t srl_loss {epoch_loss}, rl_loss {policy_loss_sum} ##############\n")
