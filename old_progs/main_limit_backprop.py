            ...
            #for batch_ids, batch_attention_masks, iter_batch_srl_labels, iter_target_s, iter_target_e, iter_target_srl in train_dataset:
                ...
                for i, (batch_srl_labels, batch_target_s, batch_target_e, batch_target_srl) in enumerate(
                    ...
                    if i != iter_size-1:
                        ...
                        start_tokens = token_embs[torch.arange(batch_size), batch_target_s].detach() # [batch, hidden]
                        ...
                        start_end_vecs = torch.concatenate(
                            [token_embs[torch.arange(batch_size), batch_target_s].detach().unsqueeze(1), srl_embs[torch.arange(batch_size), batch_target_s].unsqueeze(1),
                             token_embs[torch.arange(batch_size), batch_target_e].detach().unsqueeze(1), srl_embs[torch.arange(batch_size), batch_target_e].unsqueeze(1)], dim=2)
                        ...
                        batch_loss_e.backward(retain_graph=True)
                        batch_loss_srl = loss_function(prob_srl, batch_target_srl)
                        batch_loss_srl.backward(retain_graph=True)
