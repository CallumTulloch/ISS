...
from Evaluation.evaluate import cal_label_f1, cal_span_f1, cal_f1_for_decode, span2seq, get_pred_dic_lists
...
if __name__ == '__main__':
    ...
    """ Dataset """
    if TRAIN_FLAG:
        train_dataset, _ = mk_dataset(train_df, BATCH_SIZE, PRETRAINED_MODEL, MAX_TOKEN, lab2id)
    ...
    if TRAIN_FLAG:
        ...
        #   [{'params': srl_encoder.parameters(), 'lr': 1e-4},
        #    {'params': start_decoder.parameters(), 'lr': 1e-4},
        #    {'params': end_decoder.parameters(), 'lr': 5e-5},
        #    {'params': srl_decoder.parameters(), 'lr': 4e-5}]
        #)
        ...
        for epoch in range(100):
            ...
            with torch.no_grad():
                for batch_ids, batch_attention_masks, iter_batch_srl_labels, iter_target_s, iter_target_e, iter_target_srl in valid_dataset:
                    batch_ids, batch_attention_masks = batch_ids.to(device), batch_attention_masks.to(device)
                    init_srl_labels = iter_batch_srl_labels[0].to(device)
                    iter_target_s, iter_target_e, iter_target_srl = iter_target_s.to(device), iter_target_e.to(device), iter_target_srl.to(device)
                    ...
                    for i, (batch_target_s, batch_target_e, batch_target_srl) in enumerate(zip(iter_target_s, iter_target_e, iter_target_srl)):
                        for target_s, target_e, target_srl in zip(batch_target_s, batch_target_e, batch_target_srl):
                            if i != len(iter_target_s)-1:
                                answer.append((target_s.item(), target_e.item(), target_srl.item()))
                    ...
                    for iter in range(MAX_ITER):
                        ...
                        # 正解と予測の作成
                        prediction.append((start.item(), end.item(), srl.item()))
    ...
    sentence_encoder.eval(), srl_encoder.eval(), start_decoder.eval(), end_decoder.eval(), srl_decoder.eval()
    with torch.no_grad():
        for batch_ids, batch_attention_masks, iter_batch_srl_labels, iter_target_s, iter_target_e, iter_target_srl in tqdm(test_dataset, ncols=100):
            batch_ids, batch_attention_masks = batch_ids.to(device), batch_attention_masks.to(device)
            init_srl_labels = iter_batch_srl_labels[0].to(device)
            iter_target_s, iter_target_e, iter_target_srl = iter_target_s.to(device), iter_target_e.to(device), iter_target_srl.to(device)
